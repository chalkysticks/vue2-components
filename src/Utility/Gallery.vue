<template>
	...
	>
		<div
			class="gallery-item"
			v-for="(mediaModel, index) in media"
			v-bind:key="index"
			v-bind:class="{
				'state-active': activeIndex === index,
			}"
		>
			<slot name="before-item" v-bind:mediaModel="mediaModel" v-bind:model="model" v-bind:index="index"></slot>

			<picture>
				<img
					v-bind:class="{ 'is-loaded': loadedImages.includes(index) }"
					v-bind:src="mediaModel.getUrlLarge()"
					v-if="mediaModel?.getUrl()"
			...
			<slot name="after-item" v-bind:mediaModel="mediaModel" v-bind:model="model" v-bind:index="index"></slot>
		</div>
...
<script lang="ts">
	...
	/**
	 * Interface for any model that contains a media collection.
	 */
	export interface IHasMedia {
		media: ChalkySticks.Collection.Media;
	}
	...
	export default class UtilityGallery extends ViewBase {
		/**
		 * @return number
		 */
		...
		protected get media(): ChalkySticks.Model.Media[] {
			const MEDIA_SORT_ORDER = ['table', 'interior', 'exterior', 'person'];

			const sortedMediaModels = this.model.media.models
...
<style lang="scss">
	.utility-gallery {
		...
		border-radius: var(--chalky-utility-gallery-border-radius);
		overflow: hidden;
		position: relative;
		user-select: none;
	...
	.utility-gallery:not(.state-loaded)::before {
